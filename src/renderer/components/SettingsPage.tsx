      ...
      <div className="flex-1 overflow-auto p-4 space-y-6">
        ...
        <section>
          ...
          <div className="px-3 py-3 bg-gray-800 rounded-lg space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm text-gray-200">{t.settings.miniBarTextColor}</span>
                <p className="text-xs text-gray-500 mt-0.5">{t.settings.miniBarTextColorDesc}</p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => updateSetting('miniBarTextColor', 'white')}
                  className={`px-3 py-1 text-sm rounded transition-colors ${
                    (settings.miniBarTextColor ?? 'white') === 'white'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                  }`}
                >
                  {t.settings.miniBarTextColorWhite}
                </button>
                <button
                  onClick={() => updateSetting('miniBarTextColor', 'black')}
                  className={`px-3 py-1 text-sm rounded transition-colors ${
                    settings.miniBarTextColor === 'black'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                  }`}
                >
                  {t.settings.miniBarTextColorBlack}
                </button>
              </div>
            </div>
          </div>
